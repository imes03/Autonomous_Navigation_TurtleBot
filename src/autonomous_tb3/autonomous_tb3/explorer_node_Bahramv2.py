...
class Explorer(Node):
    ...
    def __init__(self):
        ...
        self.rrt_step_size = 0.1               #Modify depending Maze size
        self.frontier_search_radius = 0.5      #Modify depending Maze size  
        self.optimistic_distance = 0.5
 ...
 #               self.cmd_pub.publish(Twist())
    ...
    def select_rrt_frontier(self):
        ...
        for i in range(self.rrt_iterations):
            ...
            score = gain + (distance * 0.30)

            # ignore useless nodes
            if gain > 1:                                 #Modify depending Maze size
            ...
            if self.current_goal:
                ...
                if robot_pos is not None:
                    ...
                    dist_to_goal = math.hypot(
                        rx - self.current_goal[0],
                        ry - self.current_goal[1]
                    )

                    # Goal practically reached
                    elapsed = (
                        self.get_clock().now() - self.goal_start_time
                    ).nanoseconds / 1e9

                    if dist_to_goal < 0.15 and elapsed > 2.0:

                        self.get_logger().info(
                            f'Goal reached by distance ({dist_to_goal:.2f} m)'
                        )
